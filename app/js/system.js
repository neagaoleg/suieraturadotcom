$(document).ready(function() {
	$.Juitter.start({
		...
		searchObject:"neagaoleg", // needed, you can insert a username here or a word to be searched for, if you wish multiple search, separate the words by comma.
	...
	$("#cristinavicol").click(function(){
		$(".jLinks").removeClass("on");
		$(this).addClass("on");									  
		$.Juitter.start({
			searchType:"searchWord",
			searchObject:"cristinavicol",
			live:"live-120" // it will be updated every 120 seconds/2 minutes
		});
	});
	$("#ingerdemon").click(function(){
		$(".jLinks").removeClass("on");
		$(this).addClass("on");									   
		$.Juitter.start({
			searchType:"searchWord",
			searchObject:"ingerdemon",
			live:"live-20"  // it will be update every 20 seconds 
		});
	});
	$("#moscovici").click(function(){
		$(".jLinks").removeClass("on");
		$(this).addClass("on");								  
		$.Juitter.start({
			searchType:"searchWord",
			searchObject:"moscovici",
			live:"live-180" // it will be updated every 180 seconds/3 minutes
		});
	});
	$("#neothebest").click(function(){
		$(".jLinks").removeClass("on");
		$(this).addClass("on");								  
		$.Juitter.start({
			searchType:"searchWord",
			searchObject:"neothebest",
			live:"live-180" // it will be updated every 180 seconds/3 minutes
		});
	});
	$("#eugenluchianiuc").click(function(){
		$(".jLinks").removeClass("on");
		$(this).addClass("on");								  
		$.Juitter.start({
			searchType:"searchWord",
			searchObject:"eugenluchianiuc",
			live:"live-180" // it will be updated every 180 seconds/3 minutes
		});
	});
		...
		$.Juitter.start({
			...
			searchObject:"moldova " + $(".juitterSearch").val(),
